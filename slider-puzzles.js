...
function _validate(rules, state) {
    return Object.keys(rules).reduce((validation, key) => {
...
function invariantCheck(stateMachine) {
    ...
    let invariantStateMachine = {
        addInvariantRule(rule) {
            rules = rules.concat(rule);
        },
        newStateIsValid(state) {
 ...
 * @returns {boolean}
 */
function _isValidMove(puzzleWidth, move) {
    let keys = Object.keys(move);
    let distance = Math.abs(keys[0] - keys[1]);

    function isInSameRow(pos1, pos2, puzzleWidth) {
 ...
 * @private
 */
function _markCompletion(element, isComplete) {
    if (isComplete) {
...
function computeMove(state, puzzlePiece) {
 ...
 * @private
 */
function _moveTile(render, isValidMove, markCompletion, stateMachine, puzzleWidth, puzzlePiece) {
    return function () {
...
function _generatePuzzle(moveTile, tileCount, puzzleHash = []) {
    ...
    rxjs.fromEvent(tile, "click").subscribe(moveTile(tile));
    _puzzleHash[key] = tile;
    return _generatePuzzle(moveTile, tileCount - 1, _puzzleHash)
}

function _moveToState(render, markCompletion, stateMachine, velocity) {
    return function () {
        markCompletion(stateMachine.isValid());
...
function _reset(render, stateMachine) {
    return function () {
        render(stateMachine.reset());
    };
}

function _render(state, index = 0) {
...
function _randomizeState(initialState) {
    function randomize(countdown, state) {
        if (countdown <= 0) {
            return state;
        }
...
function _shuffle(randomizeState, markCompletion, render, stateMachine, completedPuzzleState) {
    return function () {
...
function addPuzzle(completedPuzzleState, puzzleContainer) {
...
function _validateTileInversions(initialState, puzzleWidth, state) {
    ...
    function countInversions(numArr = [], _puzzleWidth) {
        if (numArr.length <= 1) {
            return 0;
        }
...
class Puzzle {
    constructor(size, puzzleContainer, puzzleCompletedMessagePanel, forwardStateButton, backStateButton, resetStateButton, shuffleButton) {
        ...
        stateMachine.addValidationRule({
            one: R.curry(_isCompleted)(completedPuzzleState)
        });
        ...
        rxjs.fromEvent(forwardStateButton, "click").subscribe(moveToState(1));
        rxjs.fromEvent(backStateButton, "click").subscribe(moveToState(-1));
        rxjs.fromEvent(resetStateButton, "click").subscribe(reset);
        rxjs.fromEvent(shuffleButton, "click").subscribe(shuffle);
        puzzleCompletedMessagePanel.classList.remove("is-complete");
    }
}

rxjs.fromEvent(puzzleSizeControl, "change").subscribe((event) => {
